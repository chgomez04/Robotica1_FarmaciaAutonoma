            ...
            if self.terminarCiclo == "si":
                ...
                if self.caja == "Chico":
                    ...
                    if self.contadorChico < 18:
                        ...
                        self.posicion = 0 #atender que ciclo usar (para mover caja chica)
                        self.agrega_ciclo()
                        self.fstart_ciclo()
                    if self.contadorChico == 18:
                        self.contadorChico = 0 
                    ...
                    if self.caja == "Grande" and self.terminarCiclo == "si":
                        self.terminarCiclo = "no"
                        self.caja = ""
                        if self.contadorGrande < 18:
                            self.contadorGrande += 1
                            self.posicion = 0 #atender que ciclo usar (para mover caja grande)
                            self.agrega_ciclo()
                            self.fstart_ciclo()
                        if self.contadorGrande == 18:
                            self.contadorGrande = 0 
...
las posiciones, asi como realizar pruebas para uno o todos los motores.''')
    ...
    def create_widgets(self):
        ...
        FrEjecutar=Frame(self,width=230,height=190,bg='gray22')
        FrEjecutar.place(x=850,y=10)
        self.btn7= Button(FrEjecutar,text="Probar Ciclo",command=self.fstart_ciclo, fg="white",bg="green")
        ...
        #self.btn12=Button(FrEjecutar,text="Iniciar",command=self.iniciarP,fg="white",bg="dodger blue")
        #self.btn12.place(x=150, y=5)
        #self.btn13=Button(FrEjecutar,text="Terminar",command=self.terminarP,fg="white",bg="dodger blue")
        #self.btn13.place(x=150, y=35)
        #cantidadC = open('cantidadCiclos.txt')
        #self.listaCiclo = cantidadC.readlines()
        #cantidadC.close()
        self.cbblistaCiclo['values']= self.listaCiclo
